...
class emrExportSummaryRenderer implements emrExcelRangeRenderer
{
	...
	public function render(ilMatrixResultsExportExcel $excel, $firstRow)
	{
		...
		$this->renderMaxPoints($excel, $firstRow + 3, $this->getMaxPointsFormula(
			$excel, $firstRow, $firstCol, $lastCol
		));
	...
	{
		$this->renderStatisticRow($excel, $row, true,
			$this->getPlugin()->txt('summary_average_points'), $formula
		);
	}
	...
	{
		$this->renderStatisticRow($excel, $row, true,
			$this->getPlugin()->txt('summary_minimum_points'), $formula
		);
	}
	...
	{
		$this->renderStatisticRow($excel, $row, true,
			$this->getPlugin()->txt('summary_maximum_points'), $formula
		);
	}
	...
	{
		$this->renderStatisticRow($excel, $row, false,
